...
hyper_ranges = { "ga": np.linspace(0.1, 0.9, 100), "gsom": np.linspace(0.1, 0.9, 100),
          "l_1": np.logspace(np.log10(5), -2, 1000), "l_2_mul": np.logspace(-2, -4.5, 1000),
          "ip_mul": [2], "seed": [42] }

hyper_vals = list(ParameterSampler(hyper_ranges, n_iter=10, random_state=0))
print(hyper_vals)
        ...
        for l_1 in l_1s:
            for l_2_mul in l_2_muls:
                for ip_mul in ip_muls:
                    run_name = "%.1f_%0.1f_%0.2e_%0.2e_%.1f__%d"%(ga, gsom, l_1, l_2_mul*l_1, ip_mul, run_id)
                    run_id += 1

                    params = {"name": run_name, "seed": seeds[0], "init_sps": True, "track_sps": False, "N_train": 6000, "N_test": 600,
