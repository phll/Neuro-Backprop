...
# build run configs
runs = []
gas = np.linspace(0.1, 0.3, 3)
gsoms = np.linspace(0.1, 0.3, 3)
l_1s = np.logspace(np.log10(5), -2, 20)
l_2_muls = np.logspace(-1, -4.5, 20)
ip_muls = [1.5, 2, 2.5]
                ...
                for ip_mul in ip_muls:
                    ...
                    params = {"name": run_name, "seed": seeds[0], "init_sps": True, "N_train": 6000, "N_test": 600,
                              ...
                              "model": {"dims": [4, 120, 3], "act": "sigmoid", "dt": 0.1, "gl": 0.1, "gb": 1.0,
                                        ...
                                        "init_weights": {"up": 0.1, "down": 1, "pi": 1, "ip": 1}, "tau_w": 30, "noise": 0,
                                        "t_pattern": 100,
...
for i in range(jobs):
    ...
    f.close()
    result = subprocess.check_output('msub -N %s_%d_%d -l nodes=1:ppn=20,walltime=26:00:00,pmem=6GB job.sh "%s"'%(name, i+1, jobs, os.getcwd() + "/" + tmp + "%d.job"%(i)), shell=True)
    id = result.decode('utf-8').replace('\n', '')
    print(id)
    f_ids.write("%d\t\t%s\n"%(i, id))
f_ids.close()
