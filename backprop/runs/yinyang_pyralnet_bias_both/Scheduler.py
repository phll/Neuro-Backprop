...
for i in range(jobs):
    ...
    result = subprocess.check_output('msub -N %s_%d_%d -l nodes=1:ppn=20,walltime=37:00:00,pmem=6GB job_pyral.sh "%s"'%(name, i+1, jobs, os.getcwd() + "/" + tmp + "%d.job"%(i)), shell=True)
