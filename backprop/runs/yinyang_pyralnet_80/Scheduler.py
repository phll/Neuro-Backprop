...
for hp in hyper_vals:
    ...
    params = {"name": run_name, "seed": seed, "init_sps": True, "track_sps": False, "N_train": 6000,
              "N_test": 600, "N_val": 600, "N_epochs": 80, "val_len": 150, "vals_per_epoch": 1,
...
for i in range(jobs):
    ...
    result = subprocess.check_output('msub -N %s_%d_%d -l nodes=1:ppn=20,walltime=45:00:00,pmem=6GB job_pyral.sh "%s"'%(name, i+1, jobs, os.getcwd() + "/" + tmp + "%d.job"%(i)), shell=True)
