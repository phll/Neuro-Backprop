...
name = "yinyang_pyralnet_vary_tau_0"
        ...
        for seed in seeds:
            ...
            params = {"name": run_name, "seed": seed, "init_sps": True, "track_sps": False, "N_train": 6000,
                      ...
                      "model": {"dims": [4, 120, 3], "act": "sigmoid", "dt": 0.1, "gl": 0.1, "gb": 1.0,
                                ...
                                "t_pattern": 50,
                                "out_lag": 40, "tau_0": tau_0, "learning_lag": 0}}
...
for i in range(jobs):
    ...
    result = subprocess.check_output('msub -N %s_%d_%d -l nodes=1:ppn=20,walltime=16:00:00,pmem=6GB job_pyral.sh "%s"'%(name, i+1, jobs, os.getcwd() + "/" + tmp + "%d.job"%(i)), shell=True)
